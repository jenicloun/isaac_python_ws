...
def generate_launch_description():
    ...
    for name in robot_names:
        ...
        robot_group = GroupAction([
            ...
            # Nav2 stack
            make_nav2_group(name, params, map_file, use_sim_time, autostart),
            # AMCL
            ...
            # TF Relay
            Node(package="isaac_mapf", executable="tf_prefix_relay", name=f"{name}_tf_relay", 
                 parameters=[{"input_tf_topic": f"/{name}/tf", "output_tf_topic": "/tf", "prefix": name, "global_frames": ["World", "map", "odom"]}])
        ])
        robot_stacks.append(robot_group)
    
    
    mapf_stack = GroupAction([
    ...
    return LaunchDescription([
        ...
        # Robot
        robot_stacks[0], # carter1
        # TimerAction(period=15.0, actions=[robot_stacks[1]]), # carter2
        # TimerAction(period=30.0, actions=[robot_stacks[2]]), # carter3
        # TimerAction(period=45.0, actions=[mapf_stack])
